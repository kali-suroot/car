...
u16 Get_Adc(u8 ch)
{
	ADC_RegularChannelConfig(ADC1,ch,1,ADC_SampleTime_239Cycles5);		//通道1 规则采样顺序值1 采样时间239.5周期
	while(!ADC_GetFlagStatus(ADC1,ADC_FLAG_EOC));			//等待转换结束
	return ADC_GetConversionValue(ADC1);					//返回采样值
}
u16 Get_Adc2(void)
{
	ADC_RegularChannelConfig(ADC1,ADC_Channel_2,1,ADC_SampleTime_239Cycles5);		//通道1 规则采样顺序值1 采样时间239.5周期
	while(!ADC_GetFlagStatus(ADC1,ADC_FLAG_EOC));			//等待转换结束
	return ADC_GetConversionValue(ADC1);					//返回采样值
}